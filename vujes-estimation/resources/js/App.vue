<template>
  <v-app>
    <LoginProfileSelector v-if="selectedUser == null" :users="users" @login="loginAs" />
      ...
      <v-main class="bg-grey-lighten-4">
        <v-container class="pa-6">
          ...
          <ClientsView
            ...
            @delete="deleteClient"
          />
          <QuickAddClientModal v-model:is-open="isClientDialogOpen" @save="handleQuickAddClient" />
    ...
    computed: {
      filteredProjectsForEstimation() {
        ...
        return this.projects.filter(
          (projects) => projects.client_id == this.newEstimation.client_id
        );
      },
      ...
      async saveEstimation() {
        ...
        try {
          await api.saveEstimation(
            payload,
            this.isEditingEstimation ? this.editEstimationId : null
          );
